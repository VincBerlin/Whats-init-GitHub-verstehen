...
|---|---|---|---|---|---|---|---|---|---|---|---|
| FR-001 — remove homepage example repo line (`vercel/next.js`, `shadcn-ui/ui`, `badlogic/pi-mono`) | AC-001 | TASK-001 | TEST-001 | unit-fake | no | [canvas](canvas/homepage-tools-discovery.canvas.md) | Homepage still nudges to 3 hardcoded famous repos instead of surfacing real tools (page.tsx L61–68 still present) | USER-001 (primary) | Homepage immediately communicates the real free tools, not placeholder examples | Binary: homepage has no example-repo line (AC-001) | aligned |
| FR-002 — homepage shows Analyze + Token Calculator + Debugger as primary, functional tools | AC-002 | TASK-002 | TEST-002 | unit-fake | no | [canvas](canvas/homepage-tools-discovery.canvas.md) | Tools exist but are buried at `/tools`; homepage only advertises analyze + discovery lists | USER-001 (primary), USER-002 | Three free, local, zero-LLM-cost utilities usable without leaving the homepage | Binary: homepage exposes usable Analyze + Calculator + Debugger (AC-002) | value-risk (embed-raises-usage is ASSUMPTION, unconfirmed; RISK-004 crowding) |
| FR-003 — Daily Top renders max 3 cards | AC-003 | TASK-007 | TEST-007 | unit-fake | no | [canvas](canvas/homepage-tools-discovery.canvas.md) | Daily renders 5 (`DailyTopRepos.tsx` slice(0,5), `lg:grid-cols-5`, `getDailyTop` 5, job `rankBy(...,5)`) | USER-002 (developer), USER-001 | Honest, scannable discovery: Daily Top 3 | Binary: Daily renders ≤3 (AC-003); OBS-001 `dailyWritten:3` | aligned |
| FR-004 — Daily uses real daily ranking only if 24h delta exists; else honest preparing/sample label | AC-010 | TASK-007 | TEST-007 (+ NFR-007 sample-label) | unit-fake | no | [canvas](canvas/homepage-tools-discovery.canvas.md) | Honest fallbacks partial (`isFallback` badge + empty state) but no 24h-delta-gated label | USER-001 (primary) | Trust/honesty: no fabricated "today's movement"; initial samples labeled as samples | Binary: Daily never claims real 24h movement unless a delta exists (AC-010) | blocked (BLOCKER-002 — no fake rankings as real; must hold, cannot be assumption-laundered) |
| FR-005 — Niche Discovery renders max 5 cards | AC-004 | TASK-008 | TEST-008 | unit-fake | no | [canvas](canvas/homepage-tools-discovery.canvas.md) | Niche renders all (no slice cap); job writes 10 (`trending.ts:180` topN=10) | USER-002 (developer), USER-003 | Honest discovery: Niche Top 5 of useful smaller repos | Binary: Niche renders ≤5 (AC-004); OBS-001 `nicheWritten:5` | aligned |
| FR-006 — Niche scoring not star-dominant; hard-exclude giants >50k stars (council delta) | AC-004, TEST-009 | TASK-008 | TEST-009 | unit-fake | no | [canvas](canvas/homepage-tools-discovery.canvas.md) | `nicheQualityScore` penalizes (band=12 >20k) but does NOT exclude; threshold 20k ≠ TERM-004 50k; test only asserts a small repo *can* outrank a giant | USER-002 (developer), USER-003 | "Useful smaller repos, not famous repos" — transparent, non-star-dominant ranking | Binary: giants >50k excluded; small repo can outrank (TEST-009) | aligned (council resolved OPEN-005 → hard-exclude; align band to 50k) |
| FR-007 — Token Calculator computes locally from text/code input (no upload, no API) | AC-005 | TASK-003, TASK-004 | TEST-003, TEST-004 | unit-fake | no | [canvas](canvas/homepage-tools-discovery.canvas.md) | Calculator already real & client-side (`gpt-tokenizer`, FileReader, no upload) but not on homepage | USER-003 (founder), USER-002 | Estimate AI/token cost locally in the browser, zero LLM, no upload (privacy) | Binary: no LLM/network call from calculator (AC-008, NFR-001); tokens shown (AC-005) | aligned |
| FR-008 — Calculator shows tokens, cost, [monthly projection DEFERRED], disclaimer | AC-005 (projection part DEFERRED) | TASK-003, TASK-004 | TEST-004 | unit-fake | no | [canvas](canvas/homepage-tools-discovery.canvas.md) | Missing `monthlyRuns`/`includedCredits` inputs, `monthlyProjection` output, per-model `disclaimer`; rates are placeholders | USER-003 (founder) | Local cost estimate a founder can act on — tokens + per-model cost + visible disclaimer (projection NOT surfaced this sprint) | Binary: calculator shows tokens + cost + disclaimer (AC-005, council-amended); projection NOT a pass criterion | value-risk (MISSING-001: placeholder prices; SEC-007 no official-bill claim; projection deferred to avoid false precision) |
| FR-009 — Debugger matches Git/GitHub/Actions errors via local pattern DB (DBG-001..012, ≥12) | AC-006 | TASK-005 | TEST-005 | unit-fake | no | [canvas](canvas/homepage-tools-discovery.canvas.md) | 13 patterns exist but only git/actions tools; missing DBG: 127, 137, repo-not-found, large-file, npm-not-found, next-build-failed, railway-deploy-failed | USER-001 (primary, non-coder), USER-004 (devops) | Debug Git/GitHub/Actions errors locally and deterministically (no LLM, no chat) | Binary: debugger matches ≥12 patterns (AC-006, DoD-004); no LLM (AC-008, NFR-002) | aligned (NOGOAL-001: stays deterministic + local) |
| FR-010 — Debugger output includes cause, technical explanation, fix, risk warning, related links | AC-006 | TASK-005, TASK-006 | TEST-006 | unit-fake | no | [canvas](canvas/homepage-tools-discovery.canvas.md) | `ErrorPattern` lacks DATA-004 fields (`category`, `severity`, `technicalCause`, `fixCommands`, `riskWarning`) | USER-001 (primary), USER-004 | Cause + fix + risk warnings + links so a beginner can act safely | Binary: matched error returns cause/fix/risk/links (AC-006) | risk-introduced (RISK-002 High — destructive cmds `reset --hard`/`clean -fd`; SEC-006 severity/riskWarning required before such patterns ship) |
